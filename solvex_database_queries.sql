-- Solvex Database Queries for Admin Panel
-- Based on process-contact.php, process-dealer.php, contact.html, and dealers.html
...
-- =============================================
-- DIRECT QUERIES FOR ADMIN PANEL (No Parameters)
-- =============================================

-- 1. Get all contact submissions with status
SELECT 
    id,
...
-- 2. Get contact submissions by status
SELECT 
    id,
    CONCAT(first_name, ' ', last_name) as full_name,
    email,
...
-- 3. Get contact submission details with replies (for specific ID - replace 1 with actual ID)
SELECT 
    cs.*,
...
-- 4. Get all dealer applications with status
SELECT 
    id,
    business_name,
    contact_person,
...
-- 5. Get pending dealer applications
SELECT 
    id,
    business_name,
    contact_person,
...
-- 6. Get dealer application details with reviews (for specific ID - replace 1 with actual ID)
SELECT 
    da.*,
...
-- 8. Get dealer application statistics
SELECT 
...
-- 9. Get submissions by subject (contact form)
SELECT 
    subject,
    COUNT(*) as count
FROM contact_submissions 
...
-- 10. Get submissions by date range (last 30 days)
SELECT 
...
-- 11. Get authorized dealers by city
SELECT 
    dealer_name,
    address,
    phone,
...
-- 12. Get recent activity (last 50 activities)
SELECT 
    al.*,
    au.username,
    au.full_name
...
-- Update contact submission status (use prepared statement)
-- UPDATE contact_submissions SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?;

-- Add reply to contact submission (use prepared statement)
-- INSERT INTO contact_replies (contact_submission_id, admin_user_id, reply_message, reply_type) VALUES (?, ?, ?, ?);

-- Update contact submission as replied (use prepared statement)
-- UPDATE contact_submissions SET status = 'replied', replied_by = ?, replied_at = CURRENT_TIMESTAMP WHERE id = ?;

-- Add dealer application review (use prepared statement)
-- INSERT INTO dealer_reviews (dealer_application_id, admin_user_id, review_notes, decision) VALUES (?, ?, ?, ?);

-- Update dealer application status (use prepared statement)
-- UPDATE dealer_applications SET status = ?, reviewed_by = ?, reviewed_at = CURRENT_TIMESTAMP WHERE id = ?;

-- Search contact submissions (use prepared statement)
-- SELECT id, CONCAT(first_name, ' ', last_name) as full_name, email, subject, status, created_at
-- FROM contact_submissions 
-- WHERE first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR message LIKE ?
-- ORDER BY created_at DESC;

-- Search dealer applications (use prepared statement)
-- SELECT id, business_name, contact_person, email, phone, status, created_at
-- FROM dealer_applications 
-- WHERE business_name LIKE ? OR contact_person LIKE ? OR email LIKE ? OR address LIKE ?
-- ORDER BY created_at DESC;

-- Get contact submission by ID (use prepared statement)
-- SELECT cs.*, CONCAT(cs.first_name, ' ', cs.last_name) as full_name, COUNT(cr.id) as reply_count, au.full_name as admin_name
-- FROM contact_submissions cs
-- LEFT JOIN contact_replies cr ON cs.id = cr.contact_submission_id
-- LEFT JOIN admin_users au ON cs.replied_by = au.id
-- WHERE cs.id = ?
...
-- Get dealer application by ID (use prepared statement)
-- SELECT da.*, COUNT(dr.id) as review_count, au.full_name as admin_name
-- FROM dealer_applications da
-- LEFT JOIN dealer_reviews dr ON da.id = dr.dealer_application_id
-- LEFT JOIN admin_users au ON da.reviewed_by = au.id
-- WHERE da.id = ?
...
-- =============================================
-- GRANT ALL PRIVILEGES ON solvex_admin.* TO 'solvex_user'@'localhost' IDENTIFIED BY 'your_secure_password';
-- FLUSH PRIVILEGES; 